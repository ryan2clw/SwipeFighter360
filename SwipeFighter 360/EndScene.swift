...
class EndScene:GameScene{
   ...
   // let backgroundAudio = try? AVAudioPlayer(contentsOfURL: NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("applause", ofType: "mp3")!))
    ...
    func addWinLabel(){
        ...
        myLabel.position = CGPoint(x:CGRectGetMidX(self.frame), y:0.25 * (self.frame.height))
    ...
    override func didMoveToView(view: SKView) {
        ...
        addWinLabel()
        do{
            backgroundAudio = try AVAudioPlayer(contentsOfURL: NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("applause", ofType: "mp3")!))
        }catch{
