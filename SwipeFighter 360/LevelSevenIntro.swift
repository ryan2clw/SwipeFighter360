...
class LevelSevenIntro: GameScene {
    ...
    func addExitButton(){
        ...
        fireButton.addTarget(self, action: #selector(LevelSevenIntro.exitAction(_:)), forControlEvents: UIControlEvents.TouchDown)
  ...
  //  override func addStopButton() {
   ...
   // }
    ...
    func invaderAttack(currentTime: CFTimeInterval){
        if (currentTime - timeOfLastUpdateForInvaderAttack > 0.4) {
            addMonster()
            //loadAsteroid()
    ...
    func displayObjective(){
        ...
        myLabel.text = "Radar picked up an invading force behind"
        myLabel2.text = "those asteroids. They're sitting ducks without"
        myLabel3.text = "the destroyer nearby.  Wipe them out."
        myLabel4.text = "Looks like over 100 ships, but we got this."
