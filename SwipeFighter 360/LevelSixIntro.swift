...
class LevelSixIntro: GameScene {
    ...
    func addExitButton(){
        ...
        fireButton.addTarget(self, action: #selector(LevelSixIntro.exitAction(_:)), for: UIControlEvents.touchDown)
  ...
  //  override func addStopButton() {
   ...
   // }
    ...
    func reduceAsteroidSize(_ currentTime: CFTimeInterval){
        if currentTime - timeOfLastUpdateForScaling > 0.1{
            var rocks:[SKNode]=[]
            enumerateChildNodes(withName: "rock", using: {node,stop in rocks.append(node)})
            for rock in rocks{
